...
<table class="detail">
    ...
    <tr>
        <th>Created On</th>
        <td><?php echo Utils::escape(Utils::formatDateTime($booking->getCreatedOn())); ?></td>
    </tr>
    <tr>
        <th>Due On</th>
        <td>
            <?php if ($tooLate): ?><span class="too-late"><?php endif; ?>
            <?php echo Utils::escape(Utils::formatDateTime($booking->getDueOn())); ?>
            <?php if ($tooLate): ?></span><?php endif; ?>
        </td>
    </tr>
    <tr>
        <th>Last Modified On</th>
        <td><?php echo Utils::escape(Utils::formatDateTime($booking->getLastModifiedOn())); ?></td>
    </tr>
    <tr>
        <th>Description</th>
        <td><?php echo Utils::escape($booking->getDescription()); ?></td>
    </tr>
    <tr>
        <th>Comment</th>
        <td><?php echo Utils::escape($booking->getComment()); ?></td>
    </tr>
    <tr>
        <th>Status</th>
        <td><img src="img/status/<?php echo $booking->getStatus(); ?>.png" alt="" title="<?php echo Utils::capitalize($booking->getStatus()); ?>" class="icon" /></td>
    </tr>
    <tr>
        <td colspan="2">&nbsp;</td>
    </tr>
    <tr>
        ...
        <td>
            <div class="statuses">
            <?php foreach (Booking::allStatuses() as $status): ?>
                <?php if ($status != $booking->getStatus()): ?>
                    <a href="<?php echo Utils::createLink('change-status', array('id' => $booking->getId(), 'status' => $status)); ?>" title="Change TODO status to <?php echo Utils::capitalize($status); ?>." class="change-status-link"
            ...
            <div class="actions">
                <a href="<?php echo Utils::createLink('add-edit', array('id' => $booking->getId())); ?>"><img src="img/action/edit.png" alt="" title="Edit it." class="icon" /></a>
                <a href="<?php echo Utils::createLink('delete', array('id' => $booking->getId())); ?>" id="delete-link"><img src="img/action/delete.png" alt="" title="Delete it." class="icon" /></a>
            </div>
        </td>
    </tr>
</table>
...
<p>
    <?php $backLink = Utils::createLink('list', array('status' => $booking->getStatus())); ?>
