...
final class TodoDao {
     ...
     * @return Booking Todo or <i>null</i> if not found
     */
    ...
    /**
     * Save {@link Todo}.
     * @param Booking $todo {@link Todo} to be saved
     * @return Booking saved {@link Todo} instance
     */
    public function save(Booking $todo) {
        if ($todo->getId() === null) {
     ...
     * @throws Exception
     */
    private function insert(Booking $todo) {
        $now = new DateTime();
        $todo->setId(null);
        $todo->setCreatedOn($now);
        $todo->setLastModifiedOn($now);
        $todo->setStatus(Booking::STATUS_PENDING);
        $sql = '
     ...
     * @throws Exception
     */
    private function execute($sql, Booking $todo) {
        $statement = $this->getDb()->prepare($sql);
    ...
    private function getParams(Booking $todo) {
        $params = array(
            ':id' => $todo->getId(),
